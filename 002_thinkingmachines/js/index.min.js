'use strict';var Two=window.Two;var TWEEN=window.TWEEN;var size=6;var logo=document.querySelector('.logo');var width=logo.clientWidth;var height=logo.clientHeight;var two=new Two({type:Two.Types.canvas,width:width,height:height}).appendTo(logo);// Position helpers
var col=function col(d){return d%size};var row=function row(d){return d/size|0};var border=25;var x1=function x1(d){return col(d)*(width-border*2)/size+border};var y1=function y1(d){return row(d)*(height-border*2)/size+border};var x2=function x2(d){return(col(d)+1)*(width-border*2)/size+border};var y2=function y2(d){return(row(d)+1)*(height-border*2)/size+border};function createGrid(){var grid=[];for(var i=0;i<size*size;i++){var line=two.makeLine(x1(i),y1(i),x2(i),y2(i));line.stroke='#ffffff';line.linewidth=5;line.cap='round';grid.push(line)}return grid}// Transform flags
